...
若問題涉及醫美、美容、外觀改善或醫師認定之必要手術，請優先判斷是否屬於治療疾病/傷害、重建基本功能，或是否仍屬美容手術/外科整型除外責任；除非使用者明確詢問牙齒或牙科，否則不要以牙科手術條文作為主要判斷依據。
...
def expand_query_for_search(question: str) -> str:
    """Add policy-language synonyms for common user wording before retrieval."""
    compact = "".join(question.lower().split())
    expanded_terms: list[str] = []

    cosmetic_terms = ("醫美", "美容", "整型", "整形", "外觀", "好看", "必要手術", "治療必要")
    if any(term in compact for term in cosmetic_terms):
        expanded_terms.append(
            "美容手術 外科整型 重建基本功能 必要整型 非單純美容 除外責任"
        )

    claim_terms = ("文件不齊", "補件", "申請理賠", "理賠文件", "準備文件")
    if any(term in compact for term in claim_terms):
        expanded_terms.append("保險金申請書 診斷書 住院證明 醫療費用收據 身分證明 補件")
...
def answer_question(
    ...
    search_question = expand_query_for_search(question)
    results = vectorstore.similarity_search_with_score(search_question, k=3)
    if not results:
